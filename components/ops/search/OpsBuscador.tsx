    ...
    <div ref={rootRef}>
      ...
      <div
        className={`rounded-xl border border-zinc-200 bg-white ${
          compact ? 'sticky top-0 z-20 p-2' : 'p-3'
        }`}
      >
        <div className={compact ? undefined : 'space-y-2.5'}>
          <div className={compact ? undefined : 'flex flex-col gap-2 lg:flex-row lg:items-center'}>
          ...
          {hasFilters ? (
            <button
              type="button"
              className="inline-flex h-10 shrink-0 items-center justify-center rounded-lg border border-zinc-200 bg-white px-2.5 text-sm font-medium text-zinc-800 hover:bg-zinc-50 sm:px-3"
              aria-label={t('ops.buscador.filtersButton')}
            ...
            >
              <ListFilter className="h-4 w-4 sm:hidden" aria-hidden />
              <span className="hidden sm:inline">{t('ops.buscador.filtersButton')}</span>
              {chips.length ? <span className="ml-1 tabular-nums text-xs text-zinc-500">({chips.length})</span> : null}
              <ChevronDown className="ml-0.5 hidden h-4 w-4 sm:inline" aria-hidden />
            </button>
          ) : null}
          {hasTools ? (
            <button
          ...
          <button
            type="button"
            className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl text-zinc-400 transition hover:bg-zinc-50 hover:text-zinc-800"
            ...
            >
              ...
              {showFilters ? (
                <div className="space-y-4 sm:grid sm:grid-cols-2 sm:gap-4 sm:space-y-0">
                  ...
                  <div className="flex flex-wrap justify-end gap-2 border-t border-zinc-100 pt-3 sm:col-span-2">
